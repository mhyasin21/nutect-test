...
class PHPExcel_Calculation {
	...
	public static function _unwrapResult($value) {
		if (is_string($value)) {
/*SITS*/	//if ((isset($value[0])) && ($value[0] == '"') && (substr($value,-1) == '"')) {
/*SITS*/	if ((   /*isset($value[0])*/  strlen($value) > 0 ) && ($value[0] == '"') && (substr($value,-1) == '"')) {
...
/*SITS*/	if ((!isset($formula[0])) || ($formula[0] != '=')) return array();
		$formula = ltrim(substr($formula,1));
/*SITS*/	if (!isset($formula[0])) return array();
	...
	public function _calculateFormulaValue($formula, $cellID=null, PHPExcel_Cell $pCell = null) {
		...
		$formula = trim($formula);
/*SITS*/	if ($formula[0] != '=') return self::_wrapResult($formula);
		$formula = ltrim(substr($formula, 1));
/*SITS*/	if (!isset($formula[0])) return self::_wrapResult($formula);
...
/*SITS*/	if (($wsTitle[0] !== "\x00") && ($this->_cyclicReferenceStack->onStack($wsCellReference))) {
...
/*SITS*/		$opCharacter = $formula[$index];	//	Get the first character of the value at the current index position
//echo 'Initial character of expression block is '.$opCharacter,PHP_EOL;
/*SITS*/		if ((isset(self::$_comparisonOperators[$opCharacter])) && (strlen($formula) > $index) && (isset(self::$_comparisonOperators[$formula[$index+1]]))) {
/*SITS*/		$opCharacter .= $formula[++$index];
...
//						echo 'Element '.$localeConstant.' is an Excel Constant<br />';
			...
			//	Ignore white space
/*SITS*/	while (($formula[$index] == "\n") || ($formula[$index] == "\r")) {
				++$index;
			}
/*SITS*/	if ($formula[$index] == ' ') {
/*SITS*/		while ($formula[$index] == ' ') {
					++$index;
				}
...
/*SITS*/		} elseif ((is_numeric($token)) || ($token === NULL) || (is_bool($token)) || ($token == '') || ($token[0] == '"') || ($token[0] == '#')) {
...
/*SITS*/	if ($operand > '' && $operand[0] == '"') { $operand = self::_unwrapResult($operand); }
			//	If the string is a numeric value, we treat it as a numeric, so no further testing
			if (!is_numeric($operand)) {
				//	If not a numeric, test to see if the value is an Excel error, and so can't be used in normal binary operations
/*SITS*/		if ($operand > '' && $operand[0] == '#') {
...
/*SITS*/	if (is_string($operand1) && $operand1 > '' && $operand1[0] == '"') { $operand1 = self::_unwrapResult($operand1); }
/*SITS*/	if (is_string($operand2) && $operand2 > '' && $operand2[0] == '"') { $operand2 = self::_unwrapResult($operand2); }
